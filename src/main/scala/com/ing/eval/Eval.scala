...
class Eval(target: Option[File]) {
	...
	import com.ing.eval.Eval.jvmId
	...
	private lazy val libPath = try {
		classPathOfClass("scala.ScalaObject") // ++ classPathThisApplication()
	} catch {
		case e: Throwable =>
	...
	def apply(code: String): Unit = {
		apply[Unit](code, resetState = true)
	}
	...
	def apply[X, Y](rule: String, x: X): Y = {
		type f = X => Y
		val eval = new Eval()

		// TODO: Replace ad-hoc FQN replacement with a preprocessor
		val replaced = rule.replace("Event", "com.ing.data.Event")
			.replace("FraudResult", "com.ing.fraud.FraudResult")

		eval.apply[f](replaced)(x)
	}
	...
	def fileToClassName(f: File): String = {
	...
	/*
	   ...
	   */
	def wrapCodeInClass(className: String, code: String) = {
	...
	private def classPathOfClass(className: String): List[String] = {
		...
		if (path.indexOf("file:") >= 0) {
			...
			val result = List(path.substring(indexOfFile, indexOfSeparator))
			result
		} else {
			require(path.endsWith(resource))
			val result = List(path.substring(0, path.length - resource.length + 1))
			result
		}
	}
