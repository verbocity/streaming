...
import com.ing.streaming.journal._
import com.ing.streaming.spark.JournalListener
...
class IntegrationTest extends TestKit(ActorSystem("system"))
	...
	"A journal system" should {
		val filename = "/home/batavia/Downloads/PX141013.json"
		//createOutputFile(filename)

		val host = "localhost"
		val port = 1357
		val speed = 100.0f    // Keep equal to or below 288, which is 5 minutes for 24 hours
		...
		"Start ticking of clock" in {
			...
			new Thread { override def run() { com.ing.streaming.dashboard.PassThroughService.main(null) }}.start()
